...
var AR_SL=Ar.gC('AR_SL');var cbNtf=getid('offnotif');if(cbNtf!==null&&cbNtf.checked==false){cbNtf.click()};getclass('pTtl')[0].style='margin-top:20px';function getHostname(url){if(url.indexOf('https')>=0){var m=url.match(/^https:\/\/[^/]+/)}else if(url.indexOf('http')>=0){var m=url.match(/^http:\/\/[^/]+/)}else{var m=url.match(/^\/\/[^/]+/)}return m?m[0]:null} 
